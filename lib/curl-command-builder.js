...
function curlPassThroughAuth (securityScheme) {
  ...
  return [
    {
      headers: headers.map((header) => `-H "${header.name}: ${header.type}"`),
      params: params.map((query) => `${query.name}=${query.type}`)
    }
  ]
}
...
function curlXCustomAuth (securityScheme) {
  return [{
    headers: securityScheme.describedBy.headers.map((header) => `-H "${header.name}: ${header.type}"`)
  }]
}
