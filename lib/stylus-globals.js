  ...
  if (securitySchemes && method.securedBy) {
    method.securedBy.forEach((methodScheme) => {
      if (methodScheme) {
        ...
        commands = commands.concat(buildCurlCommands(resourceOptions, {type: 'null'}))
      }
    })
  } else {
    commands = commands.concat(buildCurlCommands(resourceOptions, {type: 'null'}))
...
function buildCurlCommands (methodParams, securityScheme) {
  ...
  let authFunction = curlAuthFunctions[securityScheme.type]
  if (authFunction === undefined) { authFunction = curlNullAuth }
