 ...
 */
function getCurlStatement (securitySchemes, baseUri, method, resource) {
  baseUri = baseUri || ''
  ...
  let commands = []
  if (securitySchemes && method.securedBy) {
    commands = buildCurlCommandsForSecuredMethod(method, securitySchemes, resourceOptions)
  }
  if (commands.length === 0) {
    commands = buildCurlCommandsForUnsecuredMethod(resourceOptions)
  }
  return commands.join('\n\n or \n\n')
}

function buildCurlCommandsForUnsecuredMethod (resourceOptions) {
  return buildCurlCommands(resourceOptions)
}

function buildCurlCommandsForSecuredMethod (method, securitySchemes, resourceOptions) {
  let commands = []
  method.securedBy.forEach((methodScheme) => {
    if (methodScheme) {
      let securityScheme = securitySchemes[methodScheme.schemeName]
      if (securityScheme) {
        commands = commands.concat(buildCurlCommands(resourceOptions, securityScheme))
      }
    } else {
      commands = buildCurlCommandsForUnsecuredMethod(resourceOptions)
    }
